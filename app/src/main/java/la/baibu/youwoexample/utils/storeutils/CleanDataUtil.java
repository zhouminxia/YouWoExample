...
public class CleanDataUtil {
     ...
     * * 清除本应用所有的数据 * *
     * 如果拍照，存入SD硬盘中，是不用删除掉
     * filepath没有自己定义的文件目录，不用传这个。
     *
     * @param context
     * @param filepath
     */
    public static void cleanApplicationData(Context context, String... filepath) {
        cleanInternalCache(context);//内部cache
        cleanDatabases(context);//内部db
        cleanSharedPreference(context);//内部sp
        cleanInternalFiles(context);//内部file
        cleanExternalCache(context);//外部cache
        if (filepath == null) {
            return;
        }
        for (String filePath : filepath) {
            cleanCustomCache(filePath);//自定义的文件目录
        }
    }
